...
class relu:
    ...
    def backward(self, X, grad):
        ...
        new_X[new_X > 0] = 1
        new_X[new_X <= 0] = 0
        backward_output = np.multiply(grad, new_X)
        return backward_output
...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        ####################################################################################################
        backward_output = np.subtract(1, np.power(np.tanh(X),2))
        backward_output = np.multiply(backward_output, grad)
        return backward_output
...
class dropout:
    ...
    def backward(self, X, grad):
        ...
        # You can use the mask created in the forward step
        ####################################################################################################
        #self.mask = self.mask * (1.0 / (1.0 - self.r))
        #self.mask = (np.random.uniform(0.0, 1.0, X.shape) >= self.r).astype(float) * (1.0 / (1.0 - self.r))
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha > 0.0:
                    ...
                    module.params[key] = np.subtract(module.params[key], _learning_rate * g)
